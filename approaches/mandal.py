...
def get_details(file_name):
    """
    Extract details from PDF using mandal approach
    Args:
    ...
    dummy_data = {
        ...
        'Amount': [1000, 2000],
        'Vendor': ['mandal Vendor 1', 'mandal Vendor 2'],
        'Description': ['Item 1 from mandal', 'Item 2 from mandal']
    }

    df = pd.DataFrame(dummy_data)
    print(f"[MANDAL] Processed {file_name} - Found {len(df)} records")
    return df
